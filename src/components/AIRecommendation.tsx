  ...
  const questions = [
    ...
    },
    {
      ...
      options: [
        ...
        { value: "international", label: "해외로 가고 싶어요", emoji: "✈️" },
    ...
    },
    {
      ...
      options: [
        { value: "sunny", label: "맑은 날씨", emoji: "☀️" },
        { value: "cool", label: "시원한 날씨", emoji: "☁️" },
    ...
    if (step < questions.length - 1) {
      ...
      setTimeout(() => {
        const filtered = marathons.filter((marathon) => {
          // 경험 레벨 매칭
          if (
            newAnswers.experience === "beginner" &&
            marathon.difficulty !== "easy"
          )
            return false;
          if (
            newAnswers.experience === "advanced" &&
            marathon.difficulty === "easy"
          )
            return false;
          ...
          // 위치 매칭
          if (
            newAnswers.location === "domestic" &&
            marathon.type !== "domestic"
          )
            return false;
          if (
            newAnswers.location === "international" &&
            marathon.type !== "international"
          )
            return false;

          // 날씨 매칭
          if (
            newAnswers.weather === "sunny" &&
            marathon.weather.condition !== "sunny"
          )
            return false;
          if (
            newAnswers.weather === "cool" &&
            marathon.weather.condition === "sunny"
          )
            return false;
            ...
            {step < questions.length ? (
              ...
              <div className="space-y-4 animate-slideIn">
                <div className="text-center mb-6">
                  <p className="text-2xl mb-2 leading-7">
                    {questions[step].options[0].emoji}
                  </p>
                  <h3 className="text-xl font-bold text-gray-800 leading-7">
                ...
                <div className="space-y-3">
                  {questions[step].options.map((option) => (
                    <button
                      key={option.value}
                      onClick={() =>
                        handleAnswer(questions[step].id, option.value)
                      }
                    ...
                    >
                      <span className="text-2xl group-hover:scale-125 transition-transform leading-7">
                        {option.emoji}
                      </span>
              ...
              <div className="space-y-4 animate-slideIn">
                <div className="text-center mb-4">
                  <h3 className="text-xl font-bold text-gray-800 mb-2 leading-7">
                    당신을 위한 추천 대회
                  </h3>
                  <p className="text-muted-foreground text-sm leading-5">
                    총 {recommendations.length}개의 대회를 찾았어요!
                  </p>
                </div>
                    ...
                    >
                      <div className="flex items-start gap-3">
                        ...
                        <div className="flex-1">
                          <h4 className="font-bold text-gray-800 mb-1 group-hover:text-primary transition-colors leading-6">
                            {marathon.name}
                          </h4>
